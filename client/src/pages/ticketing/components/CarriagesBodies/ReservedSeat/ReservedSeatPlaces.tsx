...
const ReservedSeatPlaces: React.FC<Props> = ({ places, onSelectPlace, selectedPlaces }) => {
    ...
    for (let i = 0, x = 140; i < 9; i++, x += 65) {
        ...
        const positions = [
            ...
            { x: x + 3, y: 136, index: 54 - i * 2 },
            { x: x + 37, y: 136, index: 53 - i * 2 },
        ];
        ...
        if (i === 8) bodiesElements.push(<path key={`path-end-${i}`} d={`M ${x + 65} 4 V 80 H ${x + 40}`} fill="none" stroke="dodgerblue" strokeWidth="2" strokeLinecap="square" />);
    }

    return <>{bodiesElements}</>;
}

export default ReservedSeatPlaces;
