...
export class TokenService {
  ...
  generateAccessToken(userId: number, email: string, roleId: number): string {
    return this.jwtService.sign({ userId, email, roleId }, { secret: process.env.ACCESS_TOKEN_SECRET, expiresIn: '2h' });
  }

  generateRefreshToken(userId: number, email: string, roleId: number): string {
    return this.jwtService.sign({ userId, email, roleId }, { secret: process.env.REFRESH_TOKEN_SECRET, expiresIn: '7d' });
  }
