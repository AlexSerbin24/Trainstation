...
describe('TrainService', () => {
  ...
  //findTrainForTicketing
  it('should find train details for ticketing', async () => {
    // Arrange
    const mockTrainId = 1;
    const mockRoute = { departurePoint: 'Station A', arrivalPoint: 'Station B' };

    const mockTrain = new Train();
    mockTrain.id = mockTrainId;
    mockTrain.trainNumber = 123;

    const routeSegment1 = new RouteSegment();
    const routeSegment2=  new RouteSegment();


    routeSegment1.station = {id:1,name:'Station A'};
    routeSegment2.station = { id: 2, name: 'Station B' };

    mockTrain.routeSegments = [routeSegment1,routeSegment2];
    mockTrain.trainUnits = [
      {
        id: 1,
        carriageType: CarriageType.COUPE,
        carriagePlaces: [
          {
    ...
    jest.spyOn(trainRepository, 'findOne').mockResolvedValue(mockTrain);

    // Act
    const result = await trainService.findTrainForTicketing(mockTrainId, mockRoute);

    // Assert
    expect(result).toBeDefined();
    expect(result.id).toEqual(mockTrain.id);
    expect(result.route).toEqual(mockRoute);
    expect(result.carriagesTypes).toEqual([{notOccupiedCount: 1, type: "Coupe"}, {notOccupiedCount: 2, type: "LUX"}]);
    expect(result.carriages).toHaveLength(2);
    expect(result.carriages[0].id).toEqual(mockTrain.trainUnits[0].id);
    expect(result.carriages[0].carriagePlaces).toHaveLength(2); 
    expect(result.carriages[1].id).toEqual(mockTrain.trainUnits[1].id);
    expect(result.carriages[1].carriagePlaces).toHaveLength(2); 
  });
...
it('should return trains with route passing through the specified stations and departure', async () => {
  const trainsSearchData: TrainsSearch = {
    ...
    departureDate: new Date('2024-04-15T00:00:00'),
    isRoundTrip: false
  };

  const station1:Station = {id:1, name:"Station 1"}
  const departureStationObj:Station = { id: 2, name: 'Station A' };
  const arrivalStationObj:Station = { id: 3, name: 'Station B' };

  const routeSegments: RouteSegment[] = [
    { id: 1, trainId: 1, stationId: 2, departureDate: new Date('2024-04-15T08:00:00'), arrivalDate: new Date('2024-04-15T07:55:00'), train: null, station: null },
    { id: 4, trainId: 2, stationId: 2, departureDate: new Date('2024-04-15T14:50:00'), arrivalDate: null, train: null, station: null },
  ];

  const train1 = new Train();
  train1.id = 1;
  train1.trainStatus = TrainStatus.AwaitingDeparture;
  train1.trainNumber = 1000;
  train1.trainType = TrainType.Common;
  train1.routeSegments = [
    { id: 1, trainId: 1, stationId: 2, departureDate: new Date('2024-04-15T06:00:00'), arrivalDate: null, train: null, station: departureStationObj },
    { id: 2, trainId: 1, stationId: 3, departureDate: new Date('2024-04-15T08:00:00'), arrivalDate: new Date('2024-04-15T07:55:00'), train: null, station: arrivalStationObj },
    { id: 3, trainId: 1, stationId: 1, departureDate: null, arrivalDate: new Date('2024-04-15T10:00:00'), train: null, station: station1 }
  ];
  train1.trainUnits = [
    {
      id: 1, trainId: 1,
  ...
  train2.id = 2;
  train2.trainStatus = TrainStatus.AwaitingDeparture;
  train2.trainNumber = 1001;
  train2.trainType = TrainType.Common
  train2.routeSegments = [
    { id: 4, trainId: 2, stationId: 2, departureDate: new Date('2024-04-15T14:50:00'), arrivalDate: null, train: null, station: departureStationObj },
    { id: 5, trainId: 2, stationId: 3, departureDate: new Date('2024-04-15T16:10:00'), arrivalDate: new Date('2024-04-15T16:05:00'), train: null, station: arrivalStationObj },
  ];
  ...
  // Assert
  expect(result).toEqual({ outbound: [expect.objectContaining({ id: 1 }), expect.objectContaining({ id: 2 })] });
...
it('should return empty trains array because they dont have an arrival station', async () => {
  ...
  const departureStationObj = { id: 2, name: 'Station A' };
  const arrivalStationObj = { id: 3, name: 'Station B' };

  const routeSegments: RouteSegment[] = [
    { id: 1, trainId: 1, stationId: 2, departureDate: new Date('2024-04-15T08:00:00'), arrivalDate: new Date('2024-04-15T07:55:00'), train: null, station: null },
    { id: 3, trainId: 2, stationId: 2, departureDate: new Date('2024-04-15T14:50:00'), arrivalDate: null, train: null, station: null },
  ];
  ...
  train1.trainType = TrainType.Common;
  train1.routeSegments = [
    { id: 1, trainId: 1, stationId: 2, departureDate: new Date('2024-04-15T06:00:00'), arrivalDate: null, train: null, station: null },
    { id: 2, trainId: 1, stationId: 1, departureDate: null, arrivalDate: new Date('2024-04-15T10:00:00'), train: null, station: null }
  ];
  ...
  train2.routeSegments = [
    { id: 3, trainId: 2, stationId: 2, departureDate: new Date('2024-04-15T14:50:00'), arrivalDate: null, train: null, station: null },
    { id: 4, trainId: 2, stationId: 1, departureDate: new Date('2024-04-15T16:10:00'), arrivalDate: new Date('2024-04-15T16:05:00'), train: null, station: null },
  ];
...
it('should return empty trains array because route segments with departure wont find', async () => {
  ...
  it("should update a train", async () => {
    ...
    const segments: RouteSegment[] = [{
      ...
      train: null,
      station: station1,
      departureDate: trainData.stations[0].departureDate,
      arrivalDate: trainData.stations[0].arrivalDate
    },
    {
      id: 2,
      stationId: 2,
      ...
      train: null,
      station: station2,
      departureDate: trainData.stations[1].departureDate,
      arrivalDate: trainData.stations[1].arrivalDate
    }]
