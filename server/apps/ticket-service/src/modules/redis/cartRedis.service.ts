...
import { RedisTicketDataDto, TicketDataDto } from '@app/dtos/ticket/ticket-entities.dto';
...
export class CartRedisService extends RedisClientBase {
    ...
    async getTickets(userId: number) {
        ...
        do {
            ...
            if (results.length) {
                ...
                const tickets = values.map(val => JSON.parse(val) as RedisTicketDataDto);
                keysAndValues.push(...results.map((key, index) => {
                    const {bookDate, ...ticket} = tickets[index];
                    
                    return { key, ticket, bookDate }
    ...
    async setTickets(userId: number, tickets: TicketDataDto[]) {
        ...
        for (const ticket of tickets) {
            ...
            const ticketJSON = JSON.stringify({ ...ticket, bookDate });
            ...
            keysAndValues.push({ key: ticketKey, ticket: ticket, bookDate })
